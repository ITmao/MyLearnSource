 ...
 * 2016-6-21    王兰           bug#20364 datagrid控件hideenbutton方法设置为false时，按钮未显示
 *-----------------------------------------------------------------------
 */
/**
 ...
 */
    ...
    * @ignore
	 ...
	 *	<tr><td>属性名</td>	<td>类型</td>	<td>说明</td>	<td>默认值</td></tr>
		...
		<tr><td>hidden</td>	<td>boolean</td>	<td>是否隐藏列(注意！最后一列不能设置为隐藏,隐藏列的请放移到前面即可)</td>	<td>false</td>	</tr>
		...
		<tr><td>isSort</td>	<td>boolean</td>	<td>列是否可以排序，设为true可排序,注：冻结列不能排序</td>	<td>--</td>	</tr>
		<tr><td>width</td>	<td>numbe</td>	<td>列宽	px(默认为100px,行文字大于40个字符【一个汉字相当于两个字段】时换行显示,冻结列不支持换行显示)</td>	<td>--</td>	</tr>
	 ...
	 */
	/**
	 * @description 配置表格的宽度的属性，默认是自适应宽高(不建议设置宽度)。
	 * @property width
	...
	/**
	 * @description 冻结表格标题，frozenHead属性适用不需要冻结列，但是又需要冻结标题的场景（注！冻结列frozen属性已经包含了冻结标题，无需设置frozenHead）
	 * @property frozenHead
	 * @name Horn.DataGrid#<b>frozenHead</b>
	 * @type boolean
	 * @default false
	 * @example
	 * 
...
Horn.DataGrid = Horn.extend(Horn.Base,{
	...
	init : function(dom){
		...
		if((this.params.frozen && this.params.frozen!="")||this.params.frozenHead){
			this.sortColums=this.head_dataTable.find(".u-datagrid-sorting");
		}else{
			this.sortColums=this.dataTable.find(".u-datagrid-sorting");
		}
		this.sortItem=[];
		this.sortCache=[];
		if(this.sortColums.length>0){
			
			this.sortColums.on("click",function(e){
				_this.sortClick(e);
			})
			
		}
		if((this.params.frozen && this.params.frozen!="")||this.params.frozenHead){
			this.ths = this.head_dataTable.children("tbody").find("td"); 
		}else{
			this.ths = this.dataTable.children("thead").find("td");
		}
...
//		$("div.hc-datagrid-body").height(h)
    ...
    */
	load : function(url,params){
		...
		if(this.sortItem.length>0&&this.isBackSort){
			params["sortItem"] = this.sortItem.attr("name");
...
//            _this.columnRender();
	...
	sortClick:function(e){
		...
		if(this.sortCache.length>0){
			if(this.sortItem.attr("name")!=target.attr("name")){
				this.sortItem.removeClass().addClass("u-datagrid-sorting");
				this.sortCache=this.sortItem;
			}
			
		}else{
			this.sortCache=target;
		}
		this.sortItem=target;
		this.doSort();
	},
	...
	doSort:function(){
		...
		if(this.isBackSort){
			...
			if(this.sortMode=="ASC"){
				...
				//this.sortDESC(sortName);
			}
			this.loadRecords(data);
		//	this.initPagebar(resultData);
            //被选中的数据对象init
			this.initSelect();
            //bind row click
...
//                       _this.setHeight(parseInt(_this.el.find("#data_"+_this.id).css("height")));
            ...
            if(this.params.height == undefined){//不设置表格高度的时候才这样干，如果设置了表格高度则以表格高度为准
         	   //this.setHeight(parseInt(this.el.find("#body_"+this.id).css("height")));//当不设置表格高度的时候应该自适应表格的高度，而不应该是div的高度
            }
            this.setTHHidden();
            if(_this.loadsuccess){
  	           ...
  	           if(loadsuccessFn)
  	             loadsuccessFn.apply(_this,[_this.el,data]);
  		   }
		}
	},
	
	setSortStatus:function(){
		if(this.sortItem.hasClass("u-datagrid-sorting")){
			this.sortItem.removeClass().addClass('u-datagrid-sorting-asc');
			this.sortMode="ASC";
		}
		else if(this.sortItem.hasClass("u-datagrid-sorting-asc")){
			this.sortItem.removeClass().addClass('u-datagrid-sorting-desc');
			this.sortMode="DESC";
		}
		else{
	...
	selectAll : function(){
    	...
    	//bug#21694 datagrid调用selectAll方法，列头的全选框未被选中
    	var headCheck = this.head_dataTable.find("#allcb_datagrid_id");
    	if(headCheck.length>0){
    		headCheck.prop("checked", true);
    	}
    	var froheadCheck = this.fro_ths.find("#allcb_datagrid_id");
    	if(froheadCheck.length>0){
    		froheadCheck.prop("checked", true);
    	}
            	   ...
            	   if(!resultData||$.isEmptyObject(resultData)){
	    			  ...
	    			  if(_this.loaderror){
            		   ...
            		   if(this.result&&(this.result.length==0||$.isEmptyObject(this.result))){
	                       ...
	                       if(!_this.isBackSort&&_this.sortItem.length>0){
	                    	   _this.clearSort();
	                       }
	//                       _this.setHeight(parseInt(_this.el.find("#data_"+_this.id).css("height")));
	                       //STORY #10163 [研发中心/内部需求][JresPlus][UI]datagrid组件中动态加载的表格，当初始化加载完成后后台增加数据量，表格高度没有发生变化 
		...
		//this.calMaxLength();
		 ...
		 if(this.result && this.result.length >0){
			 ...
			 for ( var i = 0; i <this.result.length; i++){
				   ...
				   if(this.frozenObj||this.params.frozenHead)
				   ...
				   for(var j=0;j<colslength;j++){
					   ...
					   if(colitem.name!=this.params["frozen"] && this.params["frozen"].indexOf(colitem.name) < 0){
						   ...
						   if((this.params.frozen&&this.params.frozen!="")||this.params.frozenHead){
							   ...
							   if(colitem["width"]&&colitem["width"]!=""){
								   tmptd = tmptd.replace("{XDATAGRID_TD_WIDTH}",colitem["width"]);
								   tmptd = tmptd.replace("{XDATAGRID_DIV_WIDTH}",colitem["width"]);
							   }
						   }
	...
	resetTdContent : function(){
    	...
    	trs.each(function(tridx,trdom){
    		...
    		for(var tdidx = 0 ; tdidx<tds.length ; tdidx++){
    			...
    			var colitem = _this.items[tdidx];
    			if(colitem){
    				tdWidth = colitem.width;
    				if(colitem.width){
    					w = colitem.width;
    					if(_this.getStrlen(text)>40){
	    					td.css("width",w+"px");
				    		tdDiv.css("width",w+"px");
			    		}
    				}
    			}
    			if(tdWidth){
    				if(widthRows.indexOf(tdidx)==-1){
			    		widthRows.push(tdidx);
			    	}
    			}
    			if(display!="none"){
		    		if(_this.getStrlen(text)>40){
			    			...
			    			if(_this.params.frozen&&_this.params.frozen!=""){
				    			tmpArray.push(trid);
				    			tdDiv.html("<textarea   readonly style=\"height: 40px;width: 100%;border: 0;background: transparent;line-height: 15px;outline: none;\" class=\"div-textarea\">"+text+"</textarea>");
    			...
    			if((_this.params.frozen&&_this.params.frozen!="")||_this.params.frozenHead){
		    		if((tdidx==tds.length-1)&&display!="none"){
		    			td.css("width","auto");
		    			tdDiv.css("width","auto");
		    		}
    			}
    		}
    	...
    	if(rows.length>0){
    		for(var i = 0;i<rows.length;i++){
    			var tmpW = 300;
    			var tdidx = widthRows[i];
    			var colitem = _this.items[tdidx];
    			if(colitem){
    				tmpW = colitem.width;
    			}
    			head_trs.find('td:eq('+rows[i]+')').css("width",tmpW);
    			head_trs.find('td:eq('+rows[i]+') div').css("width",tmpW);
    		}
    	}
	...
	dictColRender : function(td,dictName){
		...
		if(dict){
			...
			if(text=="" || dict[text]== undefined){
				...
				td.children("div").html(dict[text]||text);
	...
	hideButton:function(name,hide){
		...
		for(var i=0;i<buttons.length;i++){
			if($(buttons[i]).attr("name")==name){
				//bug#20364 datagrid控件hideenbutton方法设置为false时，按钮未显示
				if(hide){
					$(buttons[i]).hide();
				}else{
					$(buttons[i]).show();
 ...
 * 2016-6-21   王兰          需求#16314 【TS:201601070231-JRESPlus-财富管理事业部-陈为-7.文本输入框希望能够提供动态检索的功能类似百度输入框一样】
 * 2016-6-21   王兰         【bug#20326 需求19364--ie8浏览器下：textfield与textarea控件，当设置value值与emptytext时，value值未显示】
 * 2016-6-21   王兰         【ie8浏览器下，textfield与textarea控件，显示emptytext后，直接调用控件的getValue获取控件值，却获取到的是emptytext的值】
 * -----------------------------------------------------------------------
 */
/**
 * @name Horn.Field
...
Horn.Field = Horn.extend(Horn.Base,{
	...
	if(rv!=-1&&rv<10){
		if(this.params.emptyText&&this.params.emptyText!=""){
			//bug#20326 需求19364--ie8浏览器下：textfield与textarea控件，当设置value值与emptytext时，value值未显示
			if(this.params.value&&this.params.value!=""){
				this.field.val(this.params.value);
			}else{
	...
	this.img.click(function(){      	
		//模糊搜索start
		var currUrl = _field.params.url; 
		var currIsSearch = _field.params.isSearch;
		if(currIsSearch){
			if(currUrl != "" && currUrl != undefined){
				var currListDiv = $(this).parent().next();
				if(currListDiv.children().length > 0){
					_field.showList(_field.field,currListDiv);
				}
			}
		}
		//模糊搜索 end
   	...
   	this.field.keyup(function(){ 
		...
		//TS:201601070231-JRESPlus-财富管理事业部-陈为-7.文本输入框希望能够提供动态检索的功能类似百度输入框一样
		var params = {};
		params["value"] = this.value;
		params["name"] = this.name;
		var url = _field.params.url; 
		var isSearch = _field.params.isSearch;
		//是否启动textField的模糊搜索功能 同时需要配置url地址后台搜索   @wanglan
		if(isSearch){
			if(url != "" && url != undefined){
				if(this.value.length > 0){
					if(url.length > 0){
						_field.doAjax(url,params);
					}
			   	}else{
			   		var currListDiv = $("#hc_hide-textfield").children("div.hc_checkboxdiv");
					if(currListDiv.children().length > 0){
						_field.hideList(_field.field,currListDiv);
					}
			   	}
			}
		}
	...
	doAjax : function(url,data){
         ...
         success : function(resultData, textStatus, jqXHR) {
        	 var listDiv = $("#hc_hide-textfield").children("div.hc_checkboxdiv");
     	   if(resultData && resultData != 'undefined'){
     		   if(resultData.rows.length > 0){
     			   
     			   var list =[];
     			   list = resultData.rows;
     			  
     			   listDiv.empty();
     			   listDiv.append("<ul></ul>");
     			   var ul = listDiv.children("ul");
     				$.each(list, function(index, obj) {
				var li = ul.children("li[key='" + index + "']");
				if (li.length == 0) {
					var li = $("<li key='"
							+ index
							+ "' title='"
							+ obj[_this.name]
							+ "'></li>");
					var label = $("<label>"+obj[_this.name]+"</label>");
					li.append(label);
					ul.append(li) ;
				}
     				});
     				var winEl = $("div.h_floatdiv-con");
     		        if(winEl){
     		        	winEl.mousewheel(function() {
     			        	_this.hideList(_this.field,listDiv);
     			        });
     		        	winEl.scroll(function() {
     			        	_this.hideList(_this.field,listDiv);
     			        });
     		        }
     			  _this.showList(_this.field,listDiv);
     		   }else{
    			   listDiv.empty();
    			   listDiv.hide();
         	   }
	...
	showList : function(inputEl, listEl) {
	     ...
	     var pos = inputEl.offset(),
	       	listOuterHeight = inputEl.outerHeight();
		     // 显示位置
		     listEl.css("left",pos.left + 'px') ;
		     listEl.css("z-index",Horn.Window.getNextZIndex()) ;
	 ...
	 listClick : function(e) {
	     ...
	     var _li = $(e.currentTarget);
		    ...
		    this.setValue(_li.attr("title"),true,true) ;
	     
	     //window下blur事件多次触发的问题
	     this.field.trigger('blur');
	     var listDiv = _this.field.parent().next();
	     listDiv.hide();        
	 } ,
	getStrlen:function(str){//一个中文相当于两个字符
...
//        this.field.attr("readonly", readOnly);
    ...
    getValue : function() {
    	//需求 #18434 目前ABS系统中，查询条件中的textfield控件无法自动过滤录入信息前后的空格，导致客户黏贴的文字（前后有空格）无法查出信息。
   	 	var input = this.get();
        var val = input.val();
        //ie8浏览器下，textfield与textarea控件，显示emptytext后，直接调用控件的getValue获取控件值,却获取到的是emptytext的值
        var rv = this.getIEVersion();
		if(rv!=-1&&rv<10){
			if(this.params.emptyText&&this.params.emptyText!=""){
				if(val==this.params.emptyText){
					val = "";
				}
			} 
		 } 
 ...
 * 2016-6-22        王兰              bug #20328 ie8浏览器下，textfield与textarea控件，显示emptytext后，直接调用控件的getValues获取控件值,获取到的是emptytext的值
 * -----------------------------------------------------------------------
 */
/**
 * @name Horn.Form
...
//        var beforeSubmit = this.el.attr('beforeSubmit');
     ...
     */
    getValues : function(){
    	//ie8浏览器下，textfield与textarea控件，显示emptytext后，直接调用控件的getValues获取控件值,获取到的是emptytext的值
    	var rv = this.getIEVersion();
    	if(rv!=-1&&rv<10){
	        this.getFieldComps().each(function(idx,field){
	        	var val = field.el.children("input").val();
	        	var placeholder =field.el.children("input").attr("placeholder");
	        	var valArea = field.el.children("textarea").val();
	        	var placeholderArea =field.el.children("textarea").attr("placeholder");
	        	if(placeholder&&placeholder==val){
	        		field.el.children("input").val("");
	        	}
	        	if(placeholderArea&&placeholderArea==valArea){
	        		field.el.children("textarea").val("");
	        	}
	        });
    	}
    	...
    	if(rv!=-1&&rv<10){
	        this.getFieldComps().each(function(idx,field){
	        	...
	        	console.log(placeholderArea+":"+val);
	        	if(placeholder!=""&&val ==""){
	        		field.el.children("input").val(placeholder);
	        	}
	        	if(placeholderArea!=""&&valArea==""){
	        		field.el.children("textarea").val(placeholderArea);
	        	}
	        });
    	}
	    		...
	    		if( staticDict ){
	    			...
	    			if(mutidict){
    					...
    					if(text=="" || staticDict[td.text().trim()]== undefined){
							...
							tdDiv.html( staticDict[tdDiv.text().trim()] || tdDiv.text());
							td.attr("title",staticDict[tdDiv.text().trim()] || tdDiv.text());
						}
    				}
	    		}else if(buttons){
	    			td.attr("title","");
...
/**
 * 内容最大长度，超过长度的文字无法输入，一个汉字相当于两个字符。比如“中文abc”，总共的文字数为7，空格、英文字母、标点都只算一个字符。
...
//            }
		...
		isValid : function(){
	        ...
	        if(this.checkRegx.length>0){
	        	var val = this.radios.filter(":checked").val();
	        	if(val==""){
	        		rs = false;
...
Horn.Select = Horn.extend(Horn.Field,{
    ...
    init : function(dom){
        ...
        this.listEl = ref_target ?$("#hc_hide_div").children("div.hc_checkboxdiv[ref_target='" + ref_target + "']"): $("#combox_"+this.params.name);
...
//		if(isIE8){
    ...
    createHeadItem : function() {
        ...
        if(this.hasHeadItem){
        	var flag = true;
        	this.listEl.children("ul").children("li").each(function(){
        		var comName = $(this).attr("headItem");
        		if(comName&&comName==_this.name){
        			flag = false;
        			return false;
        		}
        	});
...
//            }
    ...
    onFocus : function(e) {
        ...
        var listDiv = ref_target?$("#hc_hide_div").children("div.hc_checkboxdiv[ref_target='" + ref_target + "']"):$("#combox_"+this.params.name);
...
Horn.SelectTree = Horn.extend(Horn.Field,{
	...
	initZTree : function() {
		...
		if(this.listEl.length == 0){
			...
			arrHtml.push("</div>");
			arrHtml.push("</div>") ;
			var treeDiv = $("#hc_hide-div-tree-"+name) ;
			if(treeDiv.length==0){
				treeDiv = $('<div class="hc_hide-div-tree-'+name+'" id="hc_hide_div-tree-'+name+'"></div>').appendTo("body") ;
        ...
        if(winEl){
        	winEl.mousewheel(function() {
        		$("div.hc_selectbox-tree-div").each(function(){
        			$(this).hide();
        		})
	        });
        	winEl.scroll(function() {
 ...
 */
  ...
  */
 /**
  * 启动模糊匹配后，配置url后台查询(需同时设置isSearch)
  * @name Horn.Textfield#url
  * @type String
  * @default ""
  * @example
  * #textfield({"label":"客户编号","name":"clientId","value":"","isSearch":true,"url":"/demo/query_table/myData.json"})
 */
 /**
  * 是否启动模糊匹配功能(启动模糊匹配时，需要同时配置url属性)
  * @name Horn.Textfield#isSearch
  * @type Boolean
  * @default false
  * @example
  * #textfield({"label":"客户编号","name":"clientId","value":"","isSearch":true,"url":"/demo/query_table/myData.json"})
...
Horn.TypeField = Horn.extend(Horn.Field, {
		...
		init : function(dom) {
			...
			var tipLabel = this.el.find("#u-typefield-tip-id").appendTo($("body")).html(this.params.value || "");
			var tipDiv = "";
			if(this.field.width()&& this.field.width()>0){
				tipDiv = this.el.find("#u-typefield-capital-id").width(this.field.width());
			}else{
				tipDiv = this.el.find("#u-typefield-capital-id").css('width','100px');
			}
		...
		setValue:function(value){
			...
			if(value!=null&&value!=""){
				if(this.params.value!=value){//初始化组件时不做校验 20160630 add by zhouzx
					this.validate();
				}
			}
		},
...
//				autoclose:false,
			...
			if(this.minDate!=undefined){
				input.datetimepicker("setStartDate",that.minDate);
				
			}
			if(this.maxDate!=undefined){
				input.datetimepicker("setEndDate",that.maxDate);
			}
    	...
    	if(check&&check.length>0&&!this.params.config){
			if(_field.calobj.val()!=""){
				...
				for(var i=0;i<ruleSpilt.length;i++){
					...
					ruleFunc = ruleFunc.replace("\(","").replace("\)","");
...
//					_this.calobj1 = cal1.calendar(config1);
					...
					//优化IE8效率
					if(conf.minDate!=undefined){
						_this.calobj1.datetimepicker("setStartDate",conf.minDate);
					}
					if(conf.maxDate!=undefined){
						_this.calobj1.datetimepicker("setEndDate",conf.maxDate);
					}
					/*_this.calobj1
					.datetimepicker("setStartDate",conf.minDate)
					.datetimepicker("setEndDate",conf.maxDate)
					.datetimepicker("show")*/
					_this.calobj1.datetimepicker("show");
...
//					_this.calobj2 = cal2.calendar(config2);
					...
					//优化IE8效率
					if(conf.minDate!=undefined){
						_this.calobj2.datetimepicker("setStartDate",conf.minDate);
					}
					if(conf.maxDate!=undefined){
						_this.calobj2.datetimepicker("setEndDate",conf.maxDate);
					}
					/*_this.calobj2
					.datetimepicker("setStartDate",conf.minDate)
					.datetimepicker("setEndDate",conf.maxDate)
					.datetimepicker("show")*/
					_this.calobj2.datetimepicker("show");
			...
			if(rule1&&rule1.length>0&&!this.params.config){
				if(_this.calobj1.val()!=""||_this.calobj2.val()!=""){
					...
					for(var i=0;i<ruleSpilt.length;i++){
						...
						ruleFunc = ruleFunc.replace("\(","").replace("\)","");
						if(ruleFunc!=""&&ruleFunc!="Horn.Validate.validateFmt"&&ruleFunc!=Horn.Validate.REQUIRED){
							Horn.Validate.onValid({data:[Horn.Validate,this]});
							return;
						}else{
							this.formatVal();
						}
