		...
		Horn.Frame.screen={
				...
				initTab:function(tab){
					...
					tab.click(function(event){
						...
						var currTab =$(".h-screen-con iframe").eq(i).attr("id");
					    if(i>=0){
					    	var tabBody=$(".h-screen-con iframe").eq(i);
					    	$(".h-screen-con iframe").each(function(){
					    		var id = $(this).attr("id");
					    		var iframeWin = document.getElementById(id).contentWindow;
					    		if(tabBody.length>0&&currTab==id){
					    			$(this).show();
					    			$(this).contents().find("html").css("overflow","auto");//此处兼容谷歌浏览器51版本
					    			var pos = $(this).attr("pos");
					    			if(pos){
					    				iframeWin.document.documentElement.scrollTop=pos;
					    			}
					    			var isChrome = navigator.userAgent.toLowerCase().match(/chrome/) != null;//判断是否是谷歌浏览器
					    			if(isChrome){
					    				var isFirst = true;//是否第一次滚动
					    				document.getElementById(id).contentWindow.onmousewheel =function (event) {//监听鼠标滚动事件
					    					
					    					if(isFirst){
						    					$(this)[0].document.body.scrollTop=0;
						    					$(this)[0].document.documentElement.scrollTop=0;
						    					$(this)[0].document.body.scrollTop=pos;
						    					$(this)[0].document.documentElement.scrollTop=pos;
						    					isFirst = false;
					    					}
					    					
					    				};
					    				document.getElementById(id).contentWindow.onscroll =function (event) {//监听滚动条事件
					    					if(isFirst){
						    					$(this)[0].document.body.scrollTop=0;
						    					$(this)[0].document.documentElement.scrollTop=0;
						    					$(this)[0].document.body.scrollTop=pos;
						    	    ...
						    	    if (iframeWin.document.documentElement && iframeWin.document.documentElement.scrollTop>=0) {
						    			y = iframeWin.document.documentElement.scrollTop;
						    			x = iframeWin.document.documentElement.scrollLeft;
						    	    }
						    	    $(this).attr("pos",y);
						    		$(this).contents().find("html").css("overflow","hidden");//此处兼容谷歌浏览器51版本
						    		$(this).hide();
						    	}
					    	});
					    	/*if(tabBody.length>0){
				...
				create:function(conf){
					if(Horn.Frame.screen.isLimit()){
						...
						//渲染内容
						var tabBodyHtml="<iframe tabId=\""+conf.id+"\" id=\"jresplus_iframe_"+conf.id+"\" style=\"display:none;\" frameborder=\"0\" width=\"100%\" height=\"100%\" scrolling=\"auto\" ></iframe>";
